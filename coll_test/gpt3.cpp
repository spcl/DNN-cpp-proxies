...
//#define DP_ALLREDUCE_SIZE   452984832+154389504 // num params of one shard of a layer and embedding
#define DP_ALLREDUCE_SIZE   607374336
...
int run_data_model_pipe(int grad_acc_step, int stage_id, int num_stage,
    ...
    int num_fwd_in_p1 = num_stage - stage_id;
    ...
    for(int i=0; i<num_bwd_fwd_in_p2; i++){
	if(stage_id % 2 == 0){
            if(stage_id == 0){
                MPI_Request request;
...
int main(int argc, char *argv[]){
    ...
    printf("Rank = %d, world_size = %d, layers = %d, stages = %d, acc_step = %d, total_params = %d Billions, GPT-3 DP-MP-PP runtime for each iteration = %f s\n", rank, world_size, NUM_L, num_stage, grad_acc_step, 1811939328/1024*NUM_L/1024/1024, elapse);
